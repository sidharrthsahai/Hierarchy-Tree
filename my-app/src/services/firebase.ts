const FIREBASE_DB_URL = "https://gongfetest.firebaseio.com";
...
export async function authenticateUser(
  ...
  try {
    ...
    const response = await fetch(`${FIREBASE_DB_URL}/secrets/${secret}.json`);
    const usersData = await response.json();

    const res = await fetch(`${FIREBASE_DB_URL}/users.json`);
    const users = await res.json();
    const currUser = users.find((u: FirebaseUserData) => u.id === usersData);
...
export async function getUsers(): Promise<User[]> {
  try {
    ...
    if (!Array.isArray(usersData)) {
      console.warn("Unexpected data format, expected array:", usersData);
      return [];
    }
